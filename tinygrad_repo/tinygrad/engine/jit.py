...
class TinyJit(Generic[ReturnType]):
  ...
  def __reduce__(self):
    ...
    # Save fxn when picklable so loaders can re-capture for different device (e.g. CL vs NPY).
    try:
      import pickle as _p
      _p.dumps(self.fxn)
      return self.__class__, (self.fxn, self.captured)
    except Exception:
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    if not JIT or self.cnt == 0:
      ...
      if self.captured.expected_input_info != expected_input_info:
        # Re-capture for different device when only device differs (e.g. CL vs NPY) and we have the original function.
        exp, got = self.captured.expected_input_info, expected_input_info
        # Compare by (vars_expr, dtype, device); UOp/Variable identity differs across runs
        def _slot_eq(a, b):
          return (tuple(v.expr for v in a[1]), a[2], a[3]) == (tuple(v.expr for v in b[1]), b[2], b[3])
        same_device_match = len(exp) == len(got) and all(_slot_eq(a, b) for a, b in zip(exp, got))
        if same_device_match:
          # Logically same inputs (e.g. both CL), only UOp identity differed; run captured.
          pass
        else:
          device_only_mismatch = (
            self.fxn is not None
            and len(exp) == len(got)
            and all((tuple(v.expr for v in a[1]), a[2]) == (tuple(v.expr for v in b[1]), b[2]) and a[3] != b[3] for a, b in zip(exp, got))
          )
          if device_only_mismatch:
            self.reset()
