import os, sys, pickle, time, re, functools
import numpy as np
try:
  import cloudpickle
except ImportError:
...
def _onnx_jit_forward(run_onnx, **kwargs):
  return next(iter(run_onnx({k: v.to(Device.DEFAULT) for k, v in kwargs.items()}).values())).cast("float32")
...
def compile(onnx_file):
  ...
  if not getenv("NPY_IMG"):
    # When Device.DEFAULT is CL (e.g. KA2), put all inputs on DEFAULT so JIT captures for CL (policy has no 'img').
  ...
  run_onnx_jit = TinyJit(functools.partial(_onnx_jit_forward, run_onnx), prune=True)
  for i in range(3):
  ...
  with open(OUTPUT, "wb") as f:
    try:
      import cloudpickle
      cloudpickle.dump(run_onnx_jit, f)
    except Exception:
...
if __name__ == "__main__":
  ...
  with open(OUTPUT, "rb") as f: pickle_loaded = (cloudpickle.load if cloudpickle else pickle.load)(f)
