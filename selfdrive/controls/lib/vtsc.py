...
# Dedicated, CONSERVATIVE lateral-accel budget for VTSC curve planning.
#
# This is intentionally a SEPARATE constant from drive_helpers.MAX_LATERAL_ACCEL_NO_ROLL
# (currently 4.5 m/s^2). That 4.5 is a *reactive* clamp on the curvature the lateral
# controller is allowed to command RIGHT NOW; it was raised for responsiveness. Reusing
# it here would tell VTSC "it's fine to arrive at the corner assuming we can pull 4.5g_lat",
# i.e. plan for a harsher corner, which yields a HIGHER v_safe and therefore systematically
# UNDER-slows for comfort. VTSC is a *predictive comfort* feature, so it uses a lower budget:
# ~2.7 m/s^2 sits in the widely-used comfort band (comma/sunnypilot-style vision curve
# controllers use ~2.0-3.0 m/s^2; ISO 3888-style comfortable-cornering is well under 4 m/s^2).
# Because this is shadow-only, 2.7 is a starting point to VALIDATE against logs, not a
# tuned actuation value.
VTSC_TARGET_LAT_ACCEL = 2.7  # m/s^2
