...
# REDUCTION-ONLY: lowers the cruise target the MPC drives toward. It never raises
...
VTSC_HOLD_S = 0.6               # hold the last constraint this long after it clears, so a
                                # single-frame model dropout cannot surge the target
...
class LongitudinalPlanner:
  ...
  def _apply_vtsc(self, sm, v_ego, v_cruise, reset_state):
    ...
    try:
      ...
      # rate-limit the target both ways: eases in, and releases smoothly on corner exit
