...
void can_set_orientation(bool flipped) {
  bus_config[1].bus_lookup = flipped ? 2U : 1U;
  bus_config[1].can_num_lookup = flipped ? 2U : 1U;
  bus_config[2].bus_lookup = flipped ? 1U : 2U;
  bus_config[2].can_num_lookup = flipped ? 1U : 2U;
}
...
void ignition_can_hook(CANPacket_t *msg) {
  if (msg->bus == 0U) {
    ...
    // Proton X50 prefl, need it to ignore ignition line
